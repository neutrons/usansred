...
def reportFromCSV(csvFilePath, outputFolder = None):
    ...
    xlsxWriter = pandas.ExcelWriter(
        os.path.join(outputFolder, "summary.xlsx"), 
        engine='xlsxwriter')
    ...
    mainChartUnscaled = workbook.add_chart({'type':'scatter',
                                    'subtype':'smooth_with_markers'})
    
    # Create a chart sheet for original data
    chartsheetOrig = workbook.add_chartsheet("Original")
    mainChartOrig = workbook.add_chart({'type':'scatter',
                                    'subtype':'smooth_with_markers'})
    
    # log binned data
    chartsheetLogBinned = workbook.add_chartsheet("Log Binned")
    mainChartLogBinned = workbook.add_chart({'type':'scatter',
                                    'subtype':'smooth_with_markers'})
    
    # log binned data with background removed
    chartsheetSubtracted = workbook.add_chartsheet("BG Subtracted")
    mainChartSubtracted = workbook.add_chart({'type':'scatter',
                                    'subtype':'smooth_with_markers'})
    
    
    mainChartUnscaled.set_x_axis({'name':'Q (1/A)',
                      'log_base':10 })
    
    mainChartUnscaled.set_y_axis({'name':'I (1/cn)',
                      'log_base':10 })
    mainChartUnscaled.set_title({'name':"Unscaled Data"})
    
    mainChartOrig.set_x_axis({'name':'Q (1/A)',
                      'log_base':10 })
    
    mainChartOrig.set_y_axis({'name':'I (1/cn)',
                      'log_base':10 })
    mainChartOrig.set_title({'name':"Original Data"})

    mainChartLogBinned.set_x_axis({'name':'Q (1/A)',
                      'log_base':10 })
    
    mainChartLogBinned.set_y_axis({'name':'I (1/cn)',
                      'log_base':10 })
    
    mainChartLogBinned.set_title({'name' : "Log Binned"})
    
    mainChartSubtracted.set_x_axis({'name':'Q (1/A)',
                      'log_base':10 })
    
    mainChartSubtracted.set_y_axis({'name':'I (1/cn)',
                      'log_base':10 })
            ...
            for fn in getFileNamesFromSamples(row[1]):
                ...
                if os.path.exists(ffp):
                    ...
                    if os.stat(ffp).st_size == 0:
                        ...
                        df = pandas.concat([df, df[(df["Q(1/A)"] > 0) & (df["I(1/cm)"] > 0) & (df["dI(1/cm)"] > 0)]], 
                                       ignore_index = False, axis = 1)
                        
                        #df.reset_index(drop=True, inplace=True)
                        cnames = ["Q(1/A)", "I(1/cm)", "dI(1/cm)", "Q(1/A)_positive", "I(1/cm)_positive", "dI(1/cm)_positive"]
                        df.columns = cnames
                        ...
                        chart.add_series({'name':f"{wn}",
                                          'categories': f'={wn}!$D$2:$D$100',
                                          'values' : f'={wn}!$E$2:$E$100'})
                        
                        # Add data series to the main chartsheets
                        if fn.endswith('lbs.txt'):
                            mainChartSubtracted.add_series({'name':f"{wn}",
                                                            'categories': f'={wn}!$D$2:$D$100',
                                                            'values' : f'={wn}!$E$2:$E$100'})
                        elif fn.endswith('lb.txt'):
                            mainChartLogBinned.add_series({'name':f"{wn}",
                                                           'categories': f'={wn}!$D$2:$D$100',
                                                           'values' : f'={wn}!$E$2:$E$100'})
                        elif fn.endswith('unscaled.txt'):
                            mainChartUnscaled.add_series({'name':f"{wn}",
                                                           'categories': f'={wn}!$D$2:$D$100',
                                                           'values' : f'={wn}!$E$2:$E$100'})
                        else:
                            mainChartOrig.add_series({'name':f"{wn}",
                                                      'categories': f'={wn}!$D$2:$D$100',
                                                      'values' : f'={wn}!$E$2:$E$100'})
                        
                        chart.set_x_axis({'name':f'={wn}!$A$1',
                                          'log_base':10 })
                        
                        chart.set_y_axis({'name':f'={wn}!$B$1',
                                          'log_base':10 })
