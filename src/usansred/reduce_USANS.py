...
if __name__ == "__main__":    
    ...
    for item in mtd['USANS'].getRun().getProperties():
        if item.name.startswith("BL1A:Mot:") and not item.name.endswith(".RBV"):
            ...
            or abs(stats.mean)==0 and abs(stats.standard_deviation)>0.0:
                scan_var = item.name
                ...
                for i in range(len(peaks)):
                    ...
                    if i == 0:
                        ...
                        update_sequence_info(os.path.join(outdir, "sample_%s.json" % sequence_first_run),
    ...
    json_files = [pos_json for pos_json in os.listdir(outdir) if pos_json.endswith('.json') and pos_json.startswith('scan')]
    json_files.sort()
    json_files = [os.path.join(outdir, ff) for ff in json_files]
    
    sample_json_files = [pos_json for pos_json in os.listdir(outdir) if pos_json.endswith('.json') and pos_json.startswith('sample')]
    ...
    with open(autocsvfile, 'w', newline='') as autocsv:
        ...
        for flag, title, first_run, ll, thickness in \
            zip(seq_background_flag, seq_start_runnums, seq_titles, seq_len, seq_thickness):
